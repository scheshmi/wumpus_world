...
class Wumpus_Q_Learning():

    def __init__(self, environment_rows :int , environment_columns : int, start_point : tuple, pits :list,
                wumpus: tuple , gold : tuple ,  episodes : int, epsilon : float, discount_factor : float,
    ...
    def is_terminal_state(self,row_index,col_index) -> bool:
        if self.rewards[row_index,col_index] == -1:
            return False
        else:
            return True
    
    def next_action(self , current_row_index, current_col_index , epsilon ) -> int:
    ...
    def move(self , current_row_index, current_col_index , action_index) -> tuple:
    ...
    def update_q_table(self , current_row , current_col ,action_index , reward , new_row, new_col) -> None :

         self.q_table[current_row , current_col ,action_index] = self.q_table[current_row , current_col ,action_index]  * (1 - self.lr) \
              + self.lr * ( reward + self.discount_factor * np.max(self.q_table[new_row , new_col]) )
    ...
    def get_path(self,start_point) -> list :
